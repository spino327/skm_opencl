...
void runParallel(int argc, char **argv,
	...
	memN = clCreateBuffer(context, CL_MEM_READ_ONLY |
			CL_MEM_COPY_HOST_PTR, CHN_LENGTH * sizeof(float), noise_ptr, &err);
	if(err < 0) {
		perror("Couldn't create a buffer decoding");
		exit(1);
	};
	...
	for (i = 0; i < n_csnr_vec; i++) {
		...
		for (j = 0; j < n_alpha; j++) {
			...
			for (k = 0; k < n_delta; k++) {
				...
				err |= clSetKernelArg(decoder_kernel, 4, sizeof(float), &sigman);
				err |= clSetKernelArg(decoder_kernel, 5, sizeof(float), &s_pw_sqrt);
				err |= clSetKernelArg(decoder_kernel, 6, sizeof(float), &delta_ptr[k]);
				err |= clSetKernelArg(decoder_kernel, 7, sizeof(float), &alpha_ptr[j]);
				if(err < 0) {
					perror("Couldn't create a kernel argument");
					exit(1);
				}
		...
		printf("Parallel-2 clock(sec): %f, uSec: %g\n", clock_dif_sec, time);
